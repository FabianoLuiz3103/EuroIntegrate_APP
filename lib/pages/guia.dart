...
class _GuiaScreenState extends State<GuiaScreen> {
  ...
  Future<Map<String, List<List<Pergunta>>>> _fetchData() async {
    var url1 =
        Uri.parse('$urlAPI/colaboradores/normas-departamento/${widget.id}');
  ...
  void initState() {
    ...
    _fetchPerguntas = Future.wait([
      ...
      Map<String, List<List<Pergunta>>> perguntasMap =
          results[0] as Map<String, List<List<Pergunta>>>;
      ...
      List<List<Pergunta>> todasPerguntas = [
        ...perguntasDepartamento,
        ...perguntasGerais
      ];
              ...
              child: Container(
                ...
                child: Stack(children: [
                  PageView(
                    ...
                    children: [
                      for (int i = 0;
                          i < normasCount && i < perguntasList.length;
                          i++)
                        _buildGuiaPage(
                            perguntasList[i],
                            normasAtuais[i]['nome'],
                            normasAtuais[i]['descricao'],
                            i + 1),
                    ],
                  ),
                  isDesktop
  ...
  Widget _buildGuiaPage(
      List<Pergunta> perguntas, String nome, String descricao, int guiaIndex) {
    final screenWidth = MediaQuery.of(context).size.width;
    const desktopWidthThreshold = 800.0;
    final isDesktop = screenWidth > desktopWidthThreshold;
